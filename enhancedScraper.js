...
class EnhancedNITJSRScraper {
    ...
    async initialize() {
        console.log('🚀 Initializing Enhanced NIT JSR Website Scraper...');
        if (!puppeteer) {
    ...
    async scrapePage(url, depth = 0) {
        ...
        try {
            ...
            const pageData = await this.page.evaluate(() => {
                ...
                // Extract ALL links with detailed information
            ...
            // Process links and classify them
    ...
    async processPDFDocuments() {
        if (!axios || !pdfParse) {
            console.log('📄 Skipping PDF processing (dependencies not available)');
            return;
        }
    ...
    async scrapeComprehensive() {
        try {
            ...
            // Expanded start URLs to cover more sections
            const startUrls = [
                'https://nitjsr.ac.in/',
...
if (require.main === module) {
    (async () => {
        const scraper = new EnhancedNITJSRScraper({
            maxPages: 250,  // Increased limit
            maxDepth: 4,    // Deeper crawling
            delay: 1000     // Faster scraping
        });
        
        try {
            const result = await scraper.scrapeComprehensive();
            console.log('🎉 Enhanced comprehensive scraping completed successfully!');
...
module.exports = { EnhancedNITJSRScraper };